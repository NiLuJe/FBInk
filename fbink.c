    ...
    rotate_touch_coordinates(FBInkCoordinates* coords)
{
	...
	// NOTE: But because the fun never stops, that appears to only actually be needed for Portrait orientation (odd),
	//       Landscape (even) ones match...
	//       When I say Portrait/Landscape, that's how the device *looks*, which doesn't match the FB_ROTATE_* constants...
	//       Revisit this if we ever need to match coordinates in Landscape...
