...
qt_qimageScaleAARGBA_helper_sse4(const unsigned int *pix, int xyap, int Cxy, int step, const __m128i vxyap, const __m128i vCxy)
{
                                           ...
                                           int dw, int dh, int dow, int sow)
{
    ...
    for (int y = 0; y < dh; y++) {
        ...
        for (int x = 0; x < dw; x++) {
            ...
            __m128i vx = qt_qimageScaleAARGBA_helper_sse4(sptr, yap, Cy, sow, vyap, vCy);

            int xap = xapoints[x];
            if (xap > 0) {
                const __m128i vxap = _mm_set1_epi32(xap);
                const __m128i vinvxap = _mm_sub_epi32(v256, vxap);
                __m128i vr = qt_qimageScaleAARGBA_helper_sse4(sptr + 1, yap, Cy, sow, vyap, vCy);
                                           ...
                                           int dw, int dh, int dow, int sow)
{
    ...
    for (int y = 0; y < dh; y++) {
        ...
        for (int x = 0; x < dw; x++) {
            ...
            const unsigned int *sptr = ypoints[y] + xpoints[x];
            __m128i vx = qt_qimageScaleAARGBA_helper_sse4(sptr, xap, Cx, 1, vxap, vCx);

            int yap = yapoints[y];
            if (yap > 0) {
                const __m128i vyap = _mm_set1_epi32(yap);
                const __m128i vinvyap = _mm_sub_epi32(v256, vyap);
                __m128i vr = qt_qimageScaleAARGBA_helper_sse4(sptr + sow, xap, Cx, 1, vxap, vCx);
